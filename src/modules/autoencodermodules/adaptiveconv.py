...
class SynthesisLayer(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if use_noise: # True
            self.register_buffer('noise_const', torch.randn([resolution, resolution])) # res = 16
            self.noise_strength = torch.nn.Parameter(torch.zeros([]))
        self.bias = torch.nn.Parameter(torch.zeros([out_channels])) # out_chan = 512
