...
class WaymoBase(MMDetWaymoDataset):
    ...
    def _get_patchGT(self, cam_instance, img_path, cam2img, postfix=""):
        ...
        if self.shift_center:
            ...
            shifted_patch_center = [np.clip(int(shifted_patch_center[0]), 0, WAYMO_IMG_WIDTH-1), np.clip(int(shifted_patch_center[1]), 0, WAYMO_IMG_HEIGHT-1)]
    ...
    def __getitem__(self, idx):
        ...
        cam_instances = [cam_instance for cam_instance in cam_instances if (cam_instance['bbox_label'] in self.label_ids and 
                (cam_instance['center_2d'][0] > 0. and cam_instance['center_2d'][0] < WAYMO_IMG_WIDTH))]
